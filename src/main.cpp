...
std::string processEchoInput(const std::string& input) {
    std::string result;
    bool escape = false;
...
int main() {
    ...
    while (!exit) {
        ...
        switch (isValid(command)) {
            case cd: {
                try {
                    if (args.empty() || args == "~") {
                        ...
                        const char* home = std::getenv("HOME");
                        if (home) filesystem::current_path(std::string(home) + args.substr(1));
                        else std::cerr << "cd: HOME not set\n";
                    } else filesystem::current_path(args);
            ...
            case echo: {
                std::string echoInput = input.substr(input.find(" ") + 1);
                std::string processedInput = processEchoInput(echoInput);
                std::cout << processedInput << "\n";
                break;
            }
            case exit0:
                exit = true;
